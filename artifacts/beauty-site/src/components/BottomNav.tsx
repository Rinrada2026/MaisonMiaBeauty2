  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-border z-40 pb-safe" data-testid="bottom-nav">
      <div className="flex items-center justify-between px-2 h-16">
        ...
        <Link href="/cart" className={`flex flex-col items-center justify-center w-full h-full gap-1 relative ${location === "/cart" ? "text-primary" : "text-muted-foreground"}`} data-testid="nav-cart">
          <div className="relative">
            <ShoppingCart className="w-5 h-5" strokeWidth={location === "/cart" ? 2 : 1.5} />
            {itemCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-primary text-white text-[9px] font-bold w-[14px] h-[14px] rounded-full flex items-center justify-center">
                {itemCount}
              </span>
